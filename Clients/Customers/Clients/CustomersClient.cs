...
namespace Crm.v1.Clients.Clients.Customers.Clients
{
    public class CustomersClient : ICustomersClient
    {
        ...
        public Task<Guid> CreateAsync(Customer lead, Dictionary<string, string> headers, CancellationToken ct = default)
        {
            return _httpClientFactory.PostJsonAsync<Guid>(UriBuilder.Combine(_url, "Create"), lead, headers, ct);
        }

        public Task UpdateAsync(Customer lead, Dictionary<string, string> headers, CancellationToken ct = default)
        {
            return _httpClientFactory.PatchJsonAsync(UriBuilder.Combine(_url, "Update"), lead, headers, ct);
        }
