...
namespace Crm.v1.Clients.Clients.Customers.Clients
{
    public class LeadSourcesClient : ILeadSourcesClient
    {
        ...
        public Task<LeadSource> GetAsync(Guid id, Dictionary<string, string> headers, CancellationToken ct = default)
        {
        ...
        public Task<Guid> CreateAsync(LeadSource source, Dictionary<string, string> headers, CancellationToken ct = default)
        {
            return _httpClientFactory.PutJsonAsync<Guid>(UriBuilder.Combine(_url, "Create"), source, accessToken, ct);
        }

        public Task UpdateAsync(LeadSource source, Dictionary<string, string> headers, CancellationToken ct = default)
        {
